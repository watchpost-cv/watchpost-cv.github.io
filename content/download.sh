...
ext=".tar.gz"; [ "$os" = windows ] && ext=".zip"; asset="watchpost-${version}-${os}-${arch}${ext}"; base="https://github.com/watchpost-cv/watchpost/releases/download/$version"; tmp=$(mktemp -d); trap 'rm -rf "$tmp"' EXIT INT TERM
...
case "$ext" in .zip) (cd "$tmp" && unzip -oq "$asset") ;; *) tar -xzf "$tmp/$asset" -C "$tmp" ;; esac
chmod 0755 "$tmp/watchpost"; mv "$tmp/watchpost" "$output"; echo "Downloaded Watchpost $version to $output"
