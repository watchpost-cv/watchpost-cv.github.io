...
Project status: WP01R through WP06R are complete as of 2026-08-28. Paired Linux
collectors now persist a bounded queue, retain sequence state, deliver in order,
delete only acknowledged data, and back off across disconnection. Public copy
...
## Watchpost-specific additions

This is the public website for the sibling `watchpost` application repository.
Watchpost is currently in product definition, so distinguish clearly between
design intent and implemented behaviour. Do not publish installation commands,
security guarantees, compatibility claims, screenshots, or release status until
the application repository contains matching evidence.

Keep the site visually restrained and operational: dark mode is first-class,
light and system themes remain supported, typography stays readable, and visual
elements should communicate signal, state, evidence, or system structure rather
than decoration.

The source repository tracks Nift sources and the nested `public/` repository.
After website changes:

1. run `nift build` and `nift status`;
2. inspect generated pages for unresolved Nift directives and broken paths;
3. validate JavaScript syntax and the complete diff;
4. commit generated output in `public/` first;
5. commit source and the resulting `public` repository pointer second.
